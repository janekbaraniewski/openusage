...
func (p *Provider) DashboardWidget() core.DashboardWidget {
	...
	cfg.CompactRows = []core.DashboardCompactRow{
		...
		{Label: "Tokens", Keys: []string{"context_window", "client_cli_input_tokens", "7d_tokens"}, MaxSegments: 3},
		{
			Label:       "Seats",
	...
	cfg.RawGroups = append(cfg.RawGroups,
		core.DashboardRawGroup{
			Label: "Usage Split",
			Keys: []string{
				"model_usage", "model_turns", "model_sessions", "model_tool_calls",
		...
		core.DashboardRawGroup{
			Label: "Session",
			Keys: []string{
				"last_session_model", "last_session_tokens", "last_session_repo",
