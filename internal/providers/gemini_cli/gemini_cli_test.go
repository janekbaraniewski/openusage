...
func TestFetch_ReadsLocalData(t *testing.T) {
	// Create temp directory with mock Gemini CLI data
	tmpDir := t.TempDir()
	...
	// Create conversations dir with some .pb files
...
func TestFetch_ExpiredOAuth(t *testing.T) {
	...
	// Write expired oauth_creds.json
	creds := oauthCreds{
...
func TestFetch_QuotaAPI(t *testing.T) {
	// Mock the OAuth token endpoint and CodeAssist API
	...
	// Temporarily override the package-level constants by using the test server
	// We need to set up the test to use the mock endpoints
	tmpDir := t.TempDir()

	// Write oauth_creds.json with expired token but valid refresh token
	creds := oauthCreds{
		AccessToken:  "ya29.expired",
	...
	// Test the helper functions directly with the mock server
