...
func TestProvider_Fetch_NoData(t *testing.T) {
	// Use a temp dir with no data to ensure real user data is not found
	tmpDir := t.TempDir()
	p := New()
	snap, err := p.Fetch(context.Background(), core.AccountConfig{
		ID:      "test-claude",
...
func TestEstimateCost_Opus(t *testing.T) {
	...
	// 1M * 15/M + 100K * 75/M + 500K * 1.50/M + 200K * 18.75/M
	// = 15 + 7.5 + 0.75 + 3.75 = 27.0
	expected := 27.0
...
func TestEstimateCost_Haiku(t *testing.T) {
	...
	// 1M * 0.80/M + 1M * 4.0/M = 0.8 + 4.0 = 4.8
	expected := 4.8
	if math.Abs(cost-expected) > 0.01 {
...
func TestFindPricing_Fallback(t *testing.T) {
	// Unknown model with "opus" in name should fall back to opus pricing
	...
	// Unknown model with "haiku" in name
	...
	// Completely unknown model → sonnet fallback
...
func TestParseJSONLFile(t *testing.T) {
	...
	// Verify the assistant entries have usage
	assistantCount := 0
...
// ─── Full integration test with JSONL data ───

func TestProvider_Fetch_WithJSONL(t *testing.T) {
	tmpDir := t.TempDir()

	// Create stats file (minimal)
	statsPath := filepath.Join(tmpDir, "stats-cache.json")
	...
	// Create JSONL project files with usage data
	projectDir := filepath.Join(tmpDir, "projects", "test-project-abc")
	os.MkdirAll(projectDir, 0755)
	...
	// Create several assistant messages within the current billing block
	var content string
	for i := 0; i < 5; i++ {
	...
	// Override the projects dir by creating a symlink or using the provider directly
	// We need to place the projects dir in the right location relative to home.
	// Instead, we test the readConversationJSONL method directly.
	p := New()
	...
	// Verify JSONL-derived metrics exist
	...
	// Check that daily cost was computed
	if m, ok := snap.Metrics["daily_cost_usd"]; ok {
		if m.Used == nil || *m.Used <= 0 {
			t.Errorf("Expected positive daily_cost_usd, got %v", m.Used)
		}
	} else {
		t.Error("Expected daily_cost_usd metric from JSONL data")
	}

	// Check billing block metrics
	if m, ok := snap.Metrics["block_cost_usd"]; ok {
		if m.Used == nil || *m.Used <= 0 {
			t.Errorf("Expected positive block_cost_usd, got %v", m.Used)
		}
		if m.Window != "rolling-5h" {
			t.Errorf("Expected window 'rolling-5h', got %q", m.Window)
		}
	} else {
		t.Error("Expected block_cost_usd metric")
	}

	// Block messages should be 5
	if m, ok := snap.Metrics["block_messages"]; ok {
		if m.Used == nil || *m.Used != 5 {
			t.Errorf("Expected 5 block messages, got %v", m.Used)
		}
	} else {
		t.Error("Expected block_messages metric")
	}
}

// ─── Settings reader test ───
