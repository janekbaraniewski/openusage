...
type DetectOptions struct {
	...
	// CachePath overrides the default `~/.cache/openusage/active-local.json`.
	// Mainly for tests.
	CachePath string
...
func defaultProcessLister() ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("active: listing processes: %w", err)
	}
...
func defaultCachePath() string {
	...
	return filepath.Join(home, ".cache", "openusage", "active-local.json")
}
