...
// Engine is the core scheduler that periodically polls all registered provider accounts.
type Engine struct {
	mu        sync.RWMutex
...
// NewEngine creates a new engine with the given refresh interval.
...
// RegisterProvider adds a provider adapter to the engine.
...
// SetAccounts configures which accounts should be polled.
...
// OnUpdate registers a callback for when snapshots change.
...
// RefreshAll polls every account immediately.
...
// Run starts the refresh loop. It blocks until the context is cancelled.
