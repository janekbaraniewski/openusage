...
func TestUsageMode_DetailAndGenericProviderGauges(t *testing.T) {
	...
	if !strings.Contains(detailRemaining, "Plan Remaining") {
		t.Errorf("expected detail remaining to contain 'Plan Remaining', got:\n%s", detailRemaining)
	}
	...
	if !strings.Contains(detailUsed, "Plan Used") {
		t.Errorf("expected detail used to contain 'Plan Used', got:\n%s", detailUsed)
	}
}
