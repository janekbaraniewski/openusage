...
if ! [[ "$payload" =~ [^[:space:]] ]]; then
  # Read stdin via perl — Node.js parents may leave stdin in O_NONBLOCK
  # mode, causing cat/read to busy-spin at 100% CPU. Perl clears the
  # flag, reads with proper blocking I/O, and alarm() provides a hard
  # 30s timeout with zero background processes.
